...
It is supported by major java project management tools such as and ant maven.
Through the maven plugin jacoco is extremely flexible, allowing to ignore java files and to set up threshold/limits for code quality.


Through the limits (thresholds) maven plugin configuration the CI pipeline fails when those limits are not reached. This lead to a high code quality through a significant test code coverage.

On the other hand, SonarQube integrates (amongst other quality tools) the results of the jacoco tool executions, and provides a good overview of the project quality.
...
Jacoco maven plugin is configured
...
>
>This could be a good topic for another discussion, but, in my personal opinion using lombok for regular POJOs (Java beans) does not hurt at all (or has the smallest negative impact). 
>On the contrary, it makes the code more readable and maintainable.
